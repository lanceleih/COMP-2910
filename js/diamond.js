...
function getDiamondTile(coordX, coordY) {
    for (var i = 0; i < max_row; i++) {
        for (var j = 0; j < max_col; j++) {
            ...
            var cy = (tile_height * heightFactor) / 2;
            var modx = 0;
            if (i % 2 == 1)
                modx = (uw * widthFactor);
            var x1 = coordX - (board_x * widthFactor) - (tile_width * widthFactor) * j - modx;
            var y1 = coordY - (board_y * heightFactor) - (uh * heightFactor) * i;
            if (x1 > 0 && x1 < (tile_width * widthFactor) && y1 > 0 && y1 < (tile_height * heightFactor)) {
                ...
                var d = dx / (tile_width * widthFactor) + dy / (tile_height * heightFactor);
                if (d <= 0.5) {
